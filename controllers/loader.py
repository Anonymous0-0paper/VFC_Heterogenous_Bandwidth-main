...
from controllers.zone_managers.DeepRL_BandwidthVersion.deep_rl_zone_manager import DeepRLZoneManager as DeepRLZoneManagerBandwidth
from controllers.zone_managers.DDPG_BandwidthVersion.deep_rl_zone_manager_ddpg import DeepRLZoneManager_DDPG as DeepRLZoneManager_DDPGBandwidth
from controllers.zone_managers.PPO_BandwidthVersiom.deep_rl_zone_manager_PPO import DeepRLZoneManagerPPO as DeepRLZoneManagerPPOBandwidth
from controllers.zone_managers.proportional_fair import ProportionalFairZoneManager
from controllers.zone_managers.milp import ChanceConstrainedMILPZoneManager
from controllers.zone_managers.cvar_dqn import CVaRDQNZoneManager
from utils.xml_parser import *

from utils.xml_parser import MobileNodeSumoXMLParser
...
class Loader:
    ...
    ALGORITHM_MAP: Dict[str, Type[ZoneManagerABC]] = {
        ...
        Config.ZoneManagerConfig.ALGORITHM_DEEP_RL_BANDWIDTH: DeepRLZoneManagerBandwidth,
        Config.ZoneManagerConfig.ALGORITHM_DDPG_BANDWIDTH: DeepRLZoneManager_DDPGBandwidth,
        Config.ZoneManagerConfig.ALGORITHM_STGT: STGTZoneManager,
        Config.ZoneManagerConfig.ALGORITHM_PROP_FAIR: ProportionalFairZoneManager,
        Config.ZoneManagerConfig.ALGORITHM_MILP: ChanceConstrainedMILPZoneManager,
        Config.ZoneManagerConfig.ALGORITHM_CVARDQN: CVaRDQNZoneManager,
        Config.ZoneManagerConfig.ALGORITHM_PPO_BANDWIDTH: DeepRLZoneManagerPPOBandwidth
    }
    ...
    def load_tasks_from_file(self, task_file, current_time):
        # Extract chunk number from filename
        chunk_number = int(os.path.basename(task_file).split('_')[-1].split('.')[0])
        parser = TaskSumoXMLParser(os.path.dirname(task_file), chunk_number)
        data = parser.parse()
        # Find the closest timestep (or just use all tasks)
        if not data:
            return []
        # Find the closest timestep <= current_time
        closest_step = max([step for step in data.keys() if step <= current_time], default=None)
        if closest_step is not None:
            return data[closest_step]
        # Fallback: return all tasks in the file
        all_tasks = []
        for tasks in data.values():
            all_tasks.extend(tasks)
        return all_tasks

    def load_mobile_fog_nodes_from_file(self, vehicle_file):
        # Parse the XML file for all timesteps and return the latest timestep's mobile fog nodes
        parser = MobileNodeSumoXMLParser(os.path.dirname(vehicle_file), int(os.path.basename(vehicle_file).split('_')[-1].split('.')[0]))
        data = parser.parse()
        # Get the last timestep (or you can choose the timestep you want)
        if not data:
            return {}
        last_step = max(data.keys())
        _, mobile_fog_nodes = data[last_step]
        return {node.id: node for node in mobile_fog_nodes}

    def load_user_nodes_from_file(self, vehicle_file):
        parser = MobileNodeSumoXMLParser(os.path.dirname(vehicle_file), int(os.path.basename(vehicle_file).split('_')[-1].split('.')[0]))
        data = parser.parse()
        if not data:
            return {}
        last_step = max(data.keys())
        user_nodes, _ = data[last_step]
        return {node.id: node for node in user_nodes}
