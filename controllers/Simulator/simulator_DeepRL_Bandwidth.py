...
from controllers.zone_managers.DeepRL_BandwidthVersion.deep_rl_zone_manager import DeepRLZoneManager
...
class Simulator:
    ...
    def choose_executor_and_assign(self, zone_manager_offload_task, task, current_time):
        ...
        if len(zone_manager_offload_task) != 0:
            ...
            if isinstance(chosen_zone_manager, DeepRLZoneManager):
                ...
                reward, action = chosen_zone_manager.env._compute_reward2(task, chosen_executor, current_time)
    ...
    def start_simulation(self):
        ...
        while (current_time := self.clock.get_current_time()) < Config.SimulatorConfig.SIMULATION_DURATION:
            ...
            # print(green_bg(self.user_nodes))
            ...
            for creator_id, tasks in nodes_tasks.items():
                ...
                for task in tasks:
                    ...
                    # print(green_bg(f"{task.id}: {task.creator.bandwidth}"))
    ...
    def execute_tasks_for_one_step(self):
        ...
        for node_id, node in merged_nodes.items():
            ...
            for task in tasks:
                ...
                # print(red_bg(f"{task.id}: {task.executor.id}"))
                if isinstance(task.executor, FixedFogNode) or isinstance(task.executor, MobileFogNode):
                    # print(red_bg("-----------------------------------------------"))
